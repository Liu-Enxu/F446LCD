...
{
	u16 i=0;
	for(i=0;i<256;i++)gif->bkpcolortbl[i]=gif->colortbl[i];//����ȫ����ɫ.
}
//�ָ�ȫ����ɫ��	 
//gif:gif��Ϣ;
...
{
	u16 i=0;
	for(i=0;i<256;i++)gif->colortbl[i]=gif->bkpcolortbl[i];//�ָ�ȫ����ɫ.
}
...
u8 gif_readextension(FIL *gfile,gif89a* gif, int *pTransIndex,u8 *pDisposal)
{
	...
	switch(temp)
	{
		...
		case GIF_GRAPHICCTL://ͼ�ο�����չ��
			if(gif_getdatablock(gfile,buf,4)!=4)return 1;	//ͼ�ο�����չ��ĳ��ȱ����?4 
...
u8 gif_dispimage(FIL *gfile,gif89a* gif,u16 x0,u16 y0,int Transparency, u8 Disposal) 
{
	...
	f_read(gfile,&lzwlen,1,(UINT*)&readed);//�õ�LZW����	 
...
void gif_clear2bkcolor(u16 x,u16 y,gif89a* gif,ImageScreenDescriptor pimge)
{
	...
	if(gif->gifISD.yoff>pimge.yoff)
	{
		...
		if(x0<x1&&y0<y1&&x1<320&&y1<320)pic_phy.fill(x0,y0,x1,y1,color); //�趨xy,�ķ�Χ����̫��.
	}
	...
	do
	{
		...
		{		 
			case GIF_INTRO_IMAGE://ͼ������
				...
				res=f_read(gfile,(u8*)&gif->gifISD,9,(UINT*)&readed);//��ȡһ���ֽ�
				if(res)return 1;			 
			...
			case GIF_INTRO_EXTENSION:
				...
				res=gif_readextension(gfile,gif,&TransIndex,&Disposal);//��ȡͼ����չ����Ϣ
				if(res)return 1;
	 			break;
			default:
				return 1;
	...
	if(res==0)//OK
	{
		...
		if(res==0)//���ļ�ok
		{
			...
			{	 
				...
				while(dtime--&&gifdecoding)delay_ms(1);//�ӳ�
				if(res==2)
				{
					res=0;
					break;
